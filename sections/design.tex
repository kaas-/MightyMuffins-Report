...
The initial brainstorm consists of two brainstorms that was chained together. The starting point of the first brainstorm was the point "Image to sound" and the second brainstorms starting point was "Filters". 
...
"Filters" branches out to the point called "Different Filters" which lists all the different common filters that may be applied to the audio. It also branches out to the point called "Controls". Controlling multiple filters relates to an equalizer. \todo{sæt lidt mere på sætningien}
Since it is stated in the final problem statement that it is a requirements to be able to control the filters, the point "Controls" is essential in the brainstorm. From "Controls", different methods of controlling filters were thought about, and from these "Intensity of filter", "Multiple filters at once" and "Sliders to adjust" were chosen for the design.

Based on the final problem statement the design team chose to narrow down the brainstorm, resulting  working with a static, RGB image that can be replaced at any time, then applying bandpass, comb and high shelf filters, and being able to apply these individually or at the same time with the possibility to change the intensity of them by using a peripheral device. 
The reason for choosing the specific filters is the theory behind them. Many of the listed filters are similar in the way they are built, but the chosen filters differentiate between each other but also the sound they will create after applied to an audio signal.


\section{Conceptual model}
\todo{her er foretaget nogle valg inden dette afsnit, som skal beskrives}This project will work with audiolisation of image. The initial design is focused on the conversion of images into an audio signal. \todo{beskriv gerne mere i dybden}Each pixel has an intensity value which can be converted into a range that fits the human hearing of 20 to 20.000Hz (insert refrence). The output of this operation can then be manipulated by audio processing filters (insert refrence).
